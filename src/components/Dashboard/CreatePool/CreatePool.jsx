...
import {GetAccount,bananaInstance} from '../../../hooks/GetAccount.js'
...
const CreatePools = () => {
  ...
  const createPool = async () => {
    try {
      ...
      let contract = await GetContract(addresses.fundsManager, FundsManagerContractArtifact.abi);
      if (contract) {
        setLoadStatus(true);
        const greetCallData = contract.interface.encodeFunctionData(
          "createPool",
           [getId(5), poolData.title, poolData.mapValue.length, poolData.addresses, ethers.utils.parseEther("0.01"), poolData.tokenId, poolData.address]
        )
        console.log(getId(5), poolData.title, poolData.mapValue.length, poolData.addresses, ethers.utils.parseEther("0.01"), poolData.tokenId, poolData.address);
